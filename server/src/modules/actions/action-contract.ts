...
export const actionPolicySchema = z.object({
  ...
  operations: z.array(z.literal('reassign_session_car')).length(1),
}).strict();
...
export const actionArguments = z.object({
  operation: z.string().min(1).max(64), arguments: z.record(z.unknown()),
}).strict();
...
export const proposalSchema = z.object({
  ...
  operation: z.string().min(1).max(64), summary: literalText,
