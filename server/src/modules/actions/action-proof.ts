...
export interface ActionSigner { sign(record: ActionRecord, audience: string, authorizationId: string): string }
...
export function createActionSigner(pem: string, keyId: string, clock = Date.now): ActionSigner {
  ...
  return { sign(record, audience, authorizationId) {
    ...
    const body = { iss: 'https://support-ai.pontes.uk/mcp-actions', aud: audience,
