...
foreach ($s in $specs) {
    ...
    $prompt = "Read CLAUDE.md and API-CONTRACT.md and " + $s.F + ". BEFORE building components, read the actual backend route files in server/src/modules/ to verify real request/response shapes. Implement everything. Write all tests. Run tests and fix until green. Append to progress.txt."
...
Write-Host ""
Write-Host "========== DESIGN REVIEW ==========" -ForegroundColor Magenta
...
Write-Host ""
Write-Host "========== ACCESSIBILITY REVIEW ==========" -ForegroundColor Magenta
...
npx vitest run
git log --oneline
Write-Host ("FRONTEND BUILD COMPLETE: " + (Get-Date)) -ForegroundColor Green
