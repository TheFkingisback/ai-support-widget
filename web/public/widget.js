"use strict";var AISupportWidgetModule=(()=>{var M=Object.defineProperty;var Q=Object.getOwnPropertyDescriptor;var Z=Object.getOwnPropertyNames;var ee=Object.prototype.hasOwnProperty;var te=(e,t)=>{for(var o in t)M(e,o,{get:t[o],enumerable:!0})},ae=(e,t,o,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let a of Z(t))!ee.call(e,a)&&a!==o&&M(e,a,{get:()=>t[a],enumerable:!(n=Q(t,a))||n.enumerable});return e};var ne=e=>ae(M({},"__esModule",{value:!0}),e);var pe={};te(pe,{AISupportWidget:()=>N});function I(e,t){try{let o=e.split(".")[1].replace(/-/g,"+").replace(/_/g,"/"),n=Uint8Array.from(atob(o.padEnd(Math.ceil(o.length/4)*4,"=")),i=>i.charCodeAt(0)),a=JSON.parse(new TextDecoder().decode(n));return a.tenantId!==t||typeof a.userId!="string"||!a.userId?null:JSON.stringify([t,a.userId])}catch{return null}}function B(e,t){let o=e==="light";return`
    :host {
  ...
  `}function D(e){let t=e.getJwt(),o=t;async function n(a,i,r){if(e.signal?.aborted)throw new Error("Support session closed");let s=e.getJwt();s!==o&&(t=s,o=s);let f=`${e.apiUrl}${i}`,p={Authorization:`Bearer ${t}`};r!==void 0&&(p["Content-Type"]="application/json");let u=await fetch(f,{signal:e.signal,method:a,headers:p,body:r!==void 0?JSON.stringify(r):void 0});if(u.status===401&&e.onTokenRefresh){if(t=await e.onTokenRefresh(),e.signal?.aborted)throw new Error("Support session closed");u=await fetch(f,{signal:e.signal,method:a,headers:{...p,Authorization:`Bearer ${t}`},body:r!==void 0?JSON.stringify(r):void 0})}if(!u.ok)throw await u.json().catch(()=>({statusCode:u.status,error:"UNKNOWN",message:u.statusText}));return u.json()}return{async createCase(a,i){return n("POST","/api/cases",{message:a,...i?{context:i}:{}})},async getCase(a){return n("GET",`/api/cases/${a}`)},async sendMessage(a,i){return(await n("POST",`/api/cases/${a}/messages`,{content:i})).message},async addFeedback(a,i){await n("POST",`/api/cases/${a}/feedback`,{feedback:i})},async closeCase(a,i,r){await n("POST",`/api/cases/${a}/close`,{resolution:i,rating:r})},async executeAction(a,i){return(await n("POST",`/api/cases/${a}/actions`,{action:i})).result},async escalate(a,i){return n("POST",`/api/cases/${a}/escalate`,{reason:i})}}}function H(e,t){let o=document.createElement("div");o.className="ai-evidence";for(let n of e){let a=document.createElement("span");if(a.className=`ai-evidence-badge ${n.type}`,n.type==="error_code")a.textContent=`${n.label}: ${n.value}`;else if(n.type==="job_id"){a.textContent=`${n.label}: ${n.value}`,a.setAttribute("role","button"),a.setAttribute("tabindex","0"),a.setAttribute("aria-label",`Copy ${n.label}: ${n.value}`);let i=()=>{navigator.clipboard.writeText(n.value).catch(()=>{})};a.addEventListener("click",i),a.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&(r.preventDefault(),i())})}else if(n.type==="timestamp"){let r=new Date(n.value).toLocaleString(t);a.textContent=`${n.label}: ${r}`}else n.type==="log_excerpt"?a.textContent=n.value:a.textContent=`${n.label}: ${n.value}`;o.appendChild(a)}return o}var oe=new Set(["create_ticket","request_access","custom"]);function U(e,t,o){let n=document.createElement("div");n.className="ai-actions";for(let a of e){let i=document.createElement("button");i.className=`ai-action-btn ${a.type}`,i.textContent=a.label,i.addEventListener("click",()=>{let r=()=>{ie(a,t).catch(()=>{let s=document.createElement("p");s.setAttribute("role","alert"),s.textContent="This action is unavailable. Use the application or its human support channel.",n.appendChild(s)})};oe.has(a.type)?re(o,a.label,r):r()}),n.appendChild(i)}return n}async function ie(e,t){switch(e.type){case"retry":await t.onRetry(e);break;case"open_docs":{t.onOpenDocs(e);break}case"request_access":await t.onRequestAccess(e);break;case"create_ticket":await(t.onCreateTicket??t.onCustom)(e);break;default:await t.onCustom(e)}}function re(e,t,o){let n=document.createElement("div");n.className="ai-confirm-overlay",n.setAttribute("role","alertdialog"),n.setAttribute("aria-modal","true"),n.setAttribute("aria-label",`Confirm: ${t}`);let a=document.createElement("div");a.className="ai-confirm-box";let i=document.createElement("p");i.id="ai-confirm-msg",i.textContent=`Are you sure you want to "${t}"?`,n.setAttribute("aria-describedby","ai-confirm-msg"),a.appendChild(i);let r=()=>n.remove(),s=document.createElement("button");s.className="ai-confirm-yes",s.textContent="Yes",s.addEventListener("click",()=>{r(),o()});let f=document.createElement("button");f.className="ai-confirm-no",f.textContent="Cancel",f.addEventListener("click",r),n.addEventListener("keydown",p=>{if(p.key==="Escape"){r();return}if(p.key==="Tab"){let u=n.getRootNode()instanceof ShadowRoot?n.getRootNode().activeElement:document.activeElement;p.shiftKey&&u===f?(p.preventDefault(),s.focus()):!p.shiftKey&&u===s&&(p.preventDefault(),f.focus())}}),a.appendChild(s),a.appendChild(f),n.appendChild(a),e.appendChild(n),f.focus()}function F(e,t){let o=document.createElement("div");o.className=`ai-msg ${e.role}`,o.dataset.messageId=e.id;let n=document.createElement("span");if(n.textContent=e.content,o.appendChild(n),e.evidence.length>0&&o.appendChild(H(e.evidence,t.locale)),e.actions.length>0){let a=le(t);o.appendChild(U(e.actions,a,t.panelEl))}return o}function J(e){let t=document.createElement("div");t.className="ai-case-close";let o=document.createElement("p");o.textContent="Was your issue resolved?",t.appendChild(o);let n=document.createElement("div");n.className="ai-case-close-btns";let a=document.createElement("button");a.className="ai-close-yes",a.textContent="Yes",a.setAttribute("aria-label","Issue was resolved");let i=document.createElement("button");return i.className="ai-close-no",i.textContent="No",i.setAttribute("aria-label","Issue was not resolved"),a.addEventListener("click",()=>q(t,"resolved",e)),i.addEventListener("click",()=>se(t,e)),n.appendChild(a),n.appendChild(i),t.appendChild(n),t}function se(e,t){q(e,"unresolved",t)}function q(e,t,o){e.innerHTML="";let n=document.createElement("p");n.textContent="How would you rate this experience? (1-10)",e.appendChild(n);let a=document.createElement("div");a.className="ai-rating";for(let i=1;i<=10;i++){let r=document.createElement("button");r.textContent=String(i),r.setAttribute("aria-label",`Rate ${i} out of 10`),r.addEventListener("click",()=>ce(e,t,i,o)),a.appendChild(r)}e.appendChild(a),o.messagesEl.scrollTop=o.messagesEl.scrollHeight}async function ce(e,t,o,n){e.innerHTML="";let a=document.createElement("p");a.textContent="Closing case...",e.appendChild(a);try{await n.apiClient.closeCase(n.caseId,t,o),e.innerHTML="";let i=document.createElement("p");i.textContent="Thanks for your feedback! Case closed.",e.appendChild(i),n.onCaseClosed&&setTimeout(()=>n.onCaseClosed(),2e3)}catch{e.innerHTML="";let i=document.createElement("p");i.textContent="Failed to close case. Please try again.",e.appendChild(i)}}function le(e){let{apiClient:t,caseId:o}=e;return{async onRetry(n){let a=await t.executeAction(o,n);O(a,e)},onOpenDocs(n){let a=n.payload.url;a&&window.open(a,"_blank","noopener")},async onRequestAccess(n){let a=await t.executeAction(o,n);O(a,e)},async onCustom(n){let a=await t.executeAction(o,n);O(a,e)}}}function O(e,t){let o=document.createElement("div");o.className="ai-msg system",o.textContent=e,t.messagesEl.appendChild(o),t.messagesEl.scrollTop=t.messagesEl.scrollHeight}function R(e){let{apiClient:t,locale:o,position:n,onClose:a}=e,i=n==="bottom-left",r=document.createElement("div");r.className=`ai-widget-panel${i?" left":""}`,r.setAttribute("role","dialog"),r.setAttribute("aria-label","Support chat");let s=document.createElement("div");s.className="ai-widget-header";let f=document.createElement("h2");f.className="ai-widget-header-title",f.textContent="Support",f.id="ai-widget-title",r.setAttribute("aria-labelledby","ai-widget-title");let p=document.createElement("button");p.className="ai-close-btn",p.textContent="\u2715",p.setAttribute("aria-label","Minimize support chat"),p.addEventListener("click",a);let u=document.createElement("button");u.className="ai-end-btn",u.textContent="End Session",u.setAttribute("aria-label","End support session"),u.style.display="none",s.appendChild(f),s.appendChild(u),s.appendChild(p);let l=document.createElement("div");l.className="ai-widget-messages",l.setAttribute("role","log"),l.setAttribute("aria-live","polite"),l.setAttribute("aria-label","Chat messages");let w=document.createElement("div");w.className="ai-widget-input",w.setAttribute("role","form"),w.setAttribute("aria-label","Send a message");let v=document.createElement("input");v.type="text",v.placeholder="Describe your issue...",v.setAttribute("aria-label","Type your message");let y=document.createElement("button");y.textContent="Send",y.setAttribute("aria-label","Send message"),w.appendChild(v),w.appendChild(y),r.appendChild(s),r.appendChild(l),r.appendChild(w);let g=e.initialCaseId??null,C=!1,E=!1;if(g&&(u.style.display=""),u.addEventListener("click",()=>{!g||E||(E=!0,l.appendChild(J(d())),l.scrollTop=l.scrollHeight)}),e.initialMessages&&e.initialMessages.length>0)for(let c of e.initialMessages)h(c);async function A(c){if(!g||C)return;C=!0;let k=m();try{let b=await t.sendMessage(g,c);k.remove(),h(b)}catch{k.remove();let b=document.createElement("div");b.className="ai-msg system",b.textContent="Failed to get response. Please try again.",l.appendChild(b)}finally{C=!1}}function d(){return{apiClient:t,locale:o,messagesEl:l,panelEl:r,caseId:g??"",onCaseClosed:e.onCaseClosed,onSendMessage:A}}function m(){let c=document.createElement("div");return c.className="ai-typing",c.setAttribute("role","status"),c.setAttribute("aria-live","polite"),c.textContent="AI is thinking...",l.appendChild(c),l.scrollTop=l.scrollHeight,c}function h(c){l.appendChild(F(c,d())),l.scrollTop=l.scrollHeight}async function S(){let c=v.value.trim();if(!c||C)return;C=!0,y.setAttribute("disabled",""),v.value="";let k=m();try{let b;if(g)b=await t.sendMessage(g,c);else{let T=await t.createCase(c,e.getContext?e.getContext():e.context);g=T.case.id,e.onCaseCreated?.(g),u.style.display="",b=T.aiMessage??await t.sendMessage(g,c)}k.remove();let $={id:"local_u",caseId:g,role:"user",content:c,actions:[],evidence:[],confidence:null,createdAt:new Date().toISOString()};h($),h(b)}catch{k.remove();let b=document.createElement("div");b.className="ai-msg system",b.textContent="Failed to send message. Please try again.",l.appendChild(b)}finally{C=!1,y.removeAttribute("disabled")}}return y.addEventListener("click",S),v.addEventListener("keydown",c=>{c.key==="Enter"&&S()}),r.addEventListener("keydown",c=>{c.key==="Escape"&&a()}),{element:r,destroy(){r.remove()},focus(){v.focus()},hide(){r.classList.add("hidden")},show(){r.classList.remove("hidden")}}}var de="ai_support_";function _(e){return`${de}v2_${encodeURIComponent(e)}_caseId`}function K(e,t){try{localStorage.setItem(_(e),t)}catch{}}function V(e){try{return localStorage.getItem(_(e))}catch{return null}}function Y(e){try{localStorage.removeItem(_(e))}catch{}}var N=class e{static init(t){e.instance&&e.instance.destroy();let o=t.theme??"light",n=t.position??"bottom-right",a=t.locale??"en-US",i=t.tenantKey,r=I(t.jwt,i),s=!1,f=0,p=null,u=new AbortController;try{localStorage.removeItem(`ai_support_${i}_caseId`)}catch{}let l=()=>{r&&Y(r)},w=x=>{!s&&r&&K(r,x)},v=n==="bottom-left",y=document.createElement("div");y.id="ai-support-widget",document.body.appendChild(y);let g=y.attachShadow({mode:"open"}),C=document.createElement("style");C.textContent=B(o,t.brandColor),g.appendChild(C);let E=t.jwt,A=D({apiUrl:t.apiUrl,getJwt:()=>E,signal:u.signal,onTokenRefresh:t.onTokenRefresh?async()=>{let x=await t.onTokenRefresh();return j(x),E}:void 0}),d=document.createElement("button");d.className=`ai-widget-fab${v?" left":""}`,d.textContent="\u{1F4AC}",d.setAttribute("aria-label","Open support chat"),d.setAttribute("aria-expanded","false"),d.setAttribute("aria-haspopup","dialog"),g.appendChild(d);let m=null,h=!1;function S(){l(),m&&(m.destroy(),m=null),h=!1,d.setAttribute("aria-expanded","false"),d.setAttribute("aria-label","Open support chat")}function c(x){g.appendChild(x.element),h=!0,d.setAttribute("aria-expanded","true"),d.setAttribute("aria-label","Minimize support chat"),x.focus()}async function k(){let x=f;if(t.onOpen&&await t.onOpen(),s||f!==x)return;if(m&&!h){m.show(),h=!0,d.setAttribute("aria-expanded","true"),d.setAttribute("aria-label","Minimize support chat"),m.focus();return}if(m)return;let L=r?V(r):null;if(L)try{let{case:G,messages:X}=await A.getCase(L);if(s||f!==x)return;if(G.status==="active"&&I(E,i)===r){m=R({apiClient:A,locale:a,position:n,onClose:$,onCaseClosed:S,getContext:()=>t.context,initialCaseId:L,initialMessages:X,onCaseCreated:w}),c(m);return}l()}catch{l()}s||f!==x||(m=R({apiClient:A,locale:a,position:n,onClose:$,onCaseClosed:S,getContext:()=>t.context,onCaseCreated:w}),c(m))}function b(){return s?Promise.reject(new Error("Support session closed")):p||(p=k().finally(()=>{p=null}),p)}function $(){m&&m.hide(),h=!1,d.setAttribute("aria-expanded","false"),d.setAttribute("aria-label","Open support chat"),d.focus()}function T(){f++,m&&(m.destroy(),m=null),h=!1,d.setAttribute("aria-expanded","false"),d.setAttribute("aria-label","Open support chat"),d.focus()}function P(){s=!0,u.abort(),l(),T(),y.remove(),e.instance===z&&(e.instance=null)}d.addEventListener("click",()=>{h?$():b().catch(()=>{d.setAttribute("aria-label","Support unavailable. Try again.")})});function j(x){if(s)throw new Error("Support session closed");if(I(x,i)!==r)throw P(),new Error("Support identity changed; initialize a new widget");E=x}function W(x){if(s)throw new Error("Support session closed");t.context=x}let z={open:b,close:T,destroy:P,updateJwt:j,updateContext:W};return e.instance=z,z}};N.instance=null;typeof window<"u"&&(window.AISupportWidget=N);return ne(pe);})();
